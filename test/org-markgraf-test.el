...
(ert-deftest org-markgraf-inline-preview-stops-native-playback ()
  (let ((html (org-markgraf-inline-html-document "seed 1")))
    (should (string-match-p "stopNativePlayback" html))
...
(ert-deftest org-markgraf-side-preview-mode-has-evil-style-controls ()
  ...
  (should (eq (lookup-key org-markgraf-side-preview-mode-map "H")
              #'org-markgraf-side-preview-scrub-backward-fast))
  (should (eq (lookup-key org-markgraf-side-preview-mode-map "L")
